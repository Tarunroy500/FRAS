...
from yagmail_outlook.error import YagInvalidEmailAddress
from yagmail_outlook.validate import validate_email_with_regex
from yagmail_outlook.log import get_logger
from yagmail_outlook.oauth2 import get_oauth2_info, get_oauth_string
...
class SMTP():
            ...
            for content_object, content_string in zip(content_objects,
                ...
                if content_object['main_type'] == 'image':
                    ...
                    if type(content_string) == inline:
                        ...
                        content_object['mime_object'].add_header('Content-ID', '<{0}>'.format(alias))
                        # content_object['mime_object'].add_header('Content-Disposition', 'inline')
                        ...
                        # msg_alternative.attach(content_object['mime_object'])
                    else:
                        # non-inline images get attached like any other attachment
                        msg.attach(content_object['mime_object'])

                else:
                    if content_object['encoding'] == 'quoted-printable':
                        htmlstr += '{0}'.format(content_object['mime_object'].get_payload())
                        altstr.append(content_object['mime_object'].get_payload())
    ...
    def _get_mime_object(self, content_string):
        ...
        if (content_object['main_type'] is None or
            ...
            if content_object['encoding'] != 'base64' and content_object['encoding'] != 'quoted-printable':
